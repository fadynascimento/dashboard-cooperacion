...
st.set_page_config(layout="wide", page_title="COOPERACIÓN XI - PANEL TÁCTICO", page_icon="✈️")
...
# --- CSS LIMPIO (REMOVIENDO ELEMENTOS SOBRANTES) ---
st.markdown(f"""
    <style>
    ...
    /* REMOVER ESPACIADO SUPERIOR DE STREAMLIT */
...
if df is not None:
    ...
    with c_map:
        ...
        st.markdown('<div class="title-clean">SITUACIÓN GEOGRÁFICA</div>', unsafe_allow_html=True)
        ...
        st_folium(m, width="100%", height=400, key="map_clean")
    ...
    with c_stat:
        ...
        st.markdown('<div class="title-clean" style="text-align: center;">ESTADO DE MEDIOS AÉREOS</div>', unsafe_allow_html=True)
    ...
    # --- LÍNEA 2: CRONOGRAMA ---
    st.markdown('<div class="section-card">', unsafe_allow_html=True)
    st.markdown('<div class="title-clean">CRONOGRAMA OPERATIVO (ZULU)</div>', unsafe_allow_html=True)
    ...
    if not df_t.empty:
        ...
        fig.update_layout(
            height=350, xaxis_range=[now_z - timedelta(hours=4), now_z + timedelta(hours=4)],
            margin=dict(l=10, r=10, t=50, b=10), showlegend=False,
            xaxis=dict(side="top", gridcolor="rgba(255,255,255,0.05)", title="HORA ZULU", tickformat="%H:%M"),
