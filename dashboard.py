...
def load_data(url):
    try:
        ...
        # Colunas H (7) e I (8) para os alertas
...
# --- ESTILO CSS (REMOÇÃO DE BORDAS E ALERTAS) ---
st.markdown(f"""
    <style>
...
if df is not None:
    # Lógica de Alerta (H=sim e I=Ativo)
    ...
    with col1:
        # Barra de Alerta sutil no topo do mapa
        if has_active_fire:
    ...
    with col2:
        ...
        st.dataframe(df_op[['aeronave', 'missao']].rename(columns={'missao': 'misión'}), hide_index=True, use_container_width=True, height=380)
