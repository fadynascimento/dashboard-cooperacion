...
# --- ESTILO CSS (BORDAS REMOVIDAS E AJUSTES DE ALTURA) ---
st.markdown(f"""
    <style>
...
# --- HEADER (BOLACHA AUMENTADA PARA 180PX) ---
...
if df is not None:
    # --- 1. LÍNEA DEL TIEMPO (AGORA NO TOPO) ---
    ...
    if not df_timeline.empty:
        fig = px.timeline(df_timeline, x_start="inicio_zulu", x_end="fim_zulu", y="aeronave", 
                          color="aeronave", text="COLUNA_A", template="plotly_dark", height=350)
        fig.add_vline(x=now_z, line_width=3, line_color="#ff4b4b")
        fig.update_traces(textposition='inside', insidetextanchor='middle', textfont=dict(size=12))
        
        fig.update_layout(
            showlegend=False,
            xaxis=dict(side="top", range=[now_z - timedelta(hours=6), now_z + timedelta(hours=6)],
    ...
    # --- 2. BARRA DE ALERTA E BLOCO INFERIOR (MAPA + VECTORES) ---
    ...
    col_map, col_vec = st.columns([2.5, 1])
    
    with col_map:
        f1, f2, f3 = st.columns(3)
        show_foc = f2.toggle("🔥 Focos Incendio", value=True)
        ...
        st_folium(m, width="100%", height=450, key="map_main")
    ...
    with col_vec:
        ...
        st.dataframe(df_op[['aeronave', 'missao']].rename(columns={'missao': 'misión'}), hide_index=True, use_container_width=True, height=400)

    # --- 3. DETALLE DE MISIONES (RODAPÉ) ---
