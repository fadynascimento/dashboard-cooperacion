...
def load_data(url):
    try:
        ...
        df_raw['label_timeline'] = df_raw.iloc[:, 0].astype(str) # Coluna A
...
# --- ESTILO CSS PARA POSICIONAMENTO FIXO ---
st.markdown(f"""
    <style>
    ...
    /* Container do Mapa e Tabela para evitar quebra */
    .upper-container {{
        display: flex; flex-wrap: nowrap; gap: 15px; align-items: flex-start;
    }}
...
st.markdown(f"""
    <div class="fixed-header">
        <div style="min-width: 200px;">
            <div style="font-size: 2rem; font-weight: bold; color: white; line-height: 1;">{now_z.strftime('%H:%M:%S')}Z</div>
...
if df is not None:
    # --- REDIMENSIONAMENTO DAS COLUNAS (Mapa Menor, Tabela maior/ajustada) ---
    col_map, col_table = st.columns([1.8, 1]) # Ajustado de 2.3 para 1.8
    
    with col_map:
        st.markdown('<div class="section-card">', unsafe_allow_html=True)
        m = folium.Map(location=[-18.5, -56.5], zoom_start=6, tiles='cartodbpositron', zoom_control=False)
        
        # Plotagem simplificada para performance
    ...
    with col_table:
        st.markdown('<div class="section-card" style="height: 376px;">', unsafe_allow_html=True)
        st.markdown('<p style="text-align:center; color:#00d4ff; font-weight:bold; margin-bottom:5px;">📊 VECTORES EM OPERAÇÃO</p>', unsafe_allow_html=True)
        
        df_v = df[df['LAYER'] == "Meios Aéreos"].groupby(['aeronave', 'missao'])['surtidas'].sum().reset_index()
        df_v.columns = ['AERONAVE', 'MISIÓN', 'SALIDAS']
        
        # Tabela compacta
        st.dataframe(df_v, hide_index=True, use_container_width=True, height=300)
        st.markdown('</div>', unsafe_allow_html=True)
