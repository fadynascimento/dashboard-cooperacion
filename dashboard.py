...
st.markdown(f"""
    ...
    .title-text {{
        ...
        color: white; letter-spacing: 10px; font-size: 2.2rem; font-weight: 900;
...
st.markdown(f"""
    <div class="fixed-header">
        <div style="position: absolute; left: 20px; text-align: left; border-left: 3px solid #00d4ff; padding-left: 15px;">
            ...
            <div style="color:#a3b18a; font-size:0.8rem;">Local: {now_l.strftime('%H:%M')} L</div>
        </div>
...
if df is not None:
    ...
    with c1:
        st.markdown("<h4 style='color:#00d4ff;'>📍 CONSCIÊNCIA SITUACIONAL</h4>", unsafe_allow_html=True)
    ...
    st.markdown(f"""
        <div style="text-align: center; margin-bottom: 10px; background: linear-gradient(90deg, transparent, rgba(0, 212, 255, 0.1), transparent); padding: 10px; border-radius: 20px;">
            <span style="color:#00d4ff; font-size:0.8rem; letter-spacing:2px;">REFERÊNCIA TEMPORAL DE MISSÃO</span><br>
            <span style="font-size:2rem; color:white; font-family:monospace; font-weight:bold;">{now_z.strftime('%H:%M:%S')}Z</span>
        </div>
    """, unsafe_allow_html=True)
    
    fig = px.timeline(df, x_start="inicio_zulu", x_end="fim_zulu", y="aeronave", color="aeronave", text="missao", template="plotly_dark")
    
    janela_inicio = now_z - timedelta(hours=4)
    ...
    st.error("⚠️ Erro na carga de dados.")

st.markdown('</div>', unsafe_allow_html=True)
st.markdown("""<script>setTimeout(function(){window.location.reload(1);}, 15000);</script>""", unsafe_allow_html=True)
